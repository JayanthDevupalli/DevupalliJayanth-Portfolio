...
const Projects = () => {
  ...
  return (
    <section id="projects" className="py-20 bg-gray-50 relative overflow-hidden">
      ...
      <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-br from-blue-200 via-purple-200 to-pink-200 opacity-40 z-[-2]"></div>
      <div className="absolute top-0 left-0 w-full h-full backdrop-blur-2xl z-[-1]"></div>
      ...
      <div className="container mx-auto px-6 lg:px-12">
        <motion.h2
          ...
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6 }}
          className="text-5xl font-extrabold mb-16 text-center text-gray-900 drop-shadow-md"
        >
          My Projects
        </motion.h2>
...
const ProjectCard = ({ project, index, setSelectedProject }: ProjectCardProps) => {
  ...
  return (
    <motion.div
      ...
      className="relative group rounded-3xl overflow-hidden shadow-2xl bg-white/30 backdrop-blur-md border border-white/20"
    >
      ...
      <div className="relative w-full h-64 overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent opacity-80 group-hover:opacity-60 transition duration-300"></div>
      ...
      <div className="p-6">
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          className="w-full bg-gradient-to-r from-blue-500 to-purple-600 text-white px-5 py-2.5 rounded-full font-semibold tracking-wide transition duration-300"
        >
          View Details
        </motion.button>
      </div>
            ...
            <div className="flex gap-4 overflow-x-auto pb-4 snap-x">
              {project.images.map((img, index) => (
                <motion.div
                  whileHover={{ scale: 1.05 }}
                  key={index}
                  className="flex-shrink-0 w-80 h-48 snap-center"
                >
                  <Image
                    src={img || "/placeholder.svg"}
                    alt={`${project.title} screenshot ${index + 1}`}
